...
const StylistChat: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    // Simulate API delay
    setTimeout(() => {
      const reply = "Our AI concierge is currently offline. For personalized styling advice, please contact our atelier directly on WhatsApp.";
          ...
          <div className="h-[400px] overflow-y-auto p-6 space-y-6 bg-transparent">
            {messages.map((m, idx) => (
              <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-6 py-4 text-sm leading-relaxed ${m.role === 'user'
